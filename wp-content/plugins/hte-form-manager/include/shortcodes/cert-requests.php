...
function hte_enqueue_cert_request_assets()
{
    if (is_singular() && has_shortcode(get_post()->post_content, 'hte_form_cert_request')) {
        wp_enqueue_style(
            'hte-cert-request-style',
            plugin_dir_url(dirname(__FILE__, 2)) . 'assets/css/cert-request.css',
            [],
            '1.0'
        );

        wp_enqueue_script(
            'hte-cert-request-script',
            plugin_dir_url(dirname(__FILE__, 2)) . 'assets/js/cert-request.js',
            ['jquery'],
            '1.0',
            true
        );
...
add_action('wp_enqueue_scripts', 'hte_enqueue_cert_request_assets');

// Shortcode hiển thị form chứng nhận
function hte_form_cert_request_shortcode()
{
    ob_start();
?>
...
add_shortcode('hte_form_cert_request', 'hte_form_cert_request_shortcode');

add_action('wp_ajax_hte_form_submit_cert', 'hte_form_submit_cert_handler');
add_action('wp_ajax_nopriv_hte_form_submit_cert', 'hte_form_submit_cert_handler');

function hte_form_submit_cert_handler()
{
    global $wpdb;
