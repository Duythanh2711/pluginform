...
function hte_enqueue_student_card_assets()
{
    if (is_singular() && has_shortcode(get_post()->post_content, 'hte_form_student_card')) {
        wp_enqueue_style(
            'hte-student-card-style',
            plugin_dir_url(dirname(__FILE__, 2)) . 'assets/css/student-card.css',
            [],
            '1.0'
        );

        wp_enqueue_script(
            'hte-student-card-script',
            plugin_dir_url(dirname(__FILE__, 2)) . 'assets/js/student-card.js',
            ['jquery'],
            '1.0',
            true
        );
...
add_action('wp_enqueue_scripts', 'hte_enqueue_student_card_assets');

// Shortcode hiển thị form chứng nhận
function hte_form_student_card_shortcode()
{
    ob_start();
?>
    <form method="POST" action="" class="hte_form">
        ...
        <div class="form-group">
            <label for="card_type"><span style="color: red; font-size: 20px;">*</span> Loại thẻ:</label>
            <div class="custom-card-type-group">
                <select id="card_type" name="card_type" class="form-control">
                    <option value="Cấp thẻ sinh viên (Trong thời hạn khóa học chính thức)">
                        Cấp thẻ sinh viên (Trong thời hạn khóa học chính thức)
                    </option>
                    <option value="Cấp thẻ vào phòng thi (Quá thời hạn)">
                        Cấp thẻ vào phòng thi (Quá thời hạn)
                    </option>
                </select>
            </div>
        </div>
...
add_shortcode('hte_form_student_card', 'hte_form_student_card_shortcode');

add_action('wp_ajax_hte_form_submit_student', 'hte_form_submit_student_handler');
add_action('wp_ajax_nopriv_hte_form_submit_student', 'hte_form_submit_student_handler');

function hte_form_submit_student_handler()
{
    global $wpdb;
